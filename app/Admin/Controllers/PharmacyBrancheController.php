...
class PharmacyBrancheController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'PharmacyBranche';
    ...
    {
        $grid = new Grid(new PharmacyBranche());
        ...
        $grid->column('pharmacy_id', __('Pharmacy'))->display(function ($id) {
            return "<a href='".route('admin.pharmacies.pharmacies.edit', $id)."'>Pharmacy</a>";
        })->filter();
    ...
    {
        $show = new Show(PharmacyBranche::findOrFail($id));
    ...
    {
        $form = new Form(new PharmacyBranche());

        $form->select('pharmacy_id', __('Pharmacy id'))->options(Pharmacy::all()->pluck('name_en','id'))->required();
